...
class SmoothToggleSwitch(tk.Canvas):
    ...
    def _create_switch_image(self, state):
        ...
        font_size = int(h * 0.25) # Nieco większy font lepiej wygląda
        
        # Próba załadowania fontu z listą rezerwową
        font = None
        font_paths = ["arialbd.ttf", "Arial_Bold.ttf", "DejaVuSans-Bold.ttf", "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"]
        ...
        # --- KLUCZOWA POPRAWKA: RĘCZNE CENTROWANIE ---
        # Pobieramy wymiary tekstu (lewo, góra, prawo, dół)
        bbox = draw.textbbox((0, 0), text, font=font)
        text_width = bbox[2] - bbox[0]
        text_height = bbox[3] - bbox[1]

        # Obliczamy X: dla ON po lewej, dla OFF po prawej (z marginesem)
        if state:
            # Pozycja dla ON (lewa strona przełącznika)
            text_x = (w * 0.35) - (text_width / 2)
        else:
            # Pozycja dla OFF (prawa strona przełącznika)
            text_x = (w * 0.65) - (text_width / 2)
            
        # Obliczamy Y: centrowanie pionowe
        text_y = (h / 2) - (text_height / 2) - bbox[1] # odejmujemy offset góry bboxa
