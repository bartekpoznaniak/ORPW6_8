...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Słownik, który przechowuje stan (True/False) dla KAŻDEGO obiektu przycisku
        self.button_states = {} 

        # Tworzenie 4 przycisków w pętli
        for i in range(4):
            btn = ctk.CTkButton(
                self,
                image=self.img_on,
                text=f"Przycisk {i+1}", # Dodajemy tekst identyfikujący przycisk
                width=150,
                height=40,
                compound="right", # Tekst obok obrazka
                fg_color=self.bg_color,
                hover_color=self.bg_color,
                hover=False,
                border_width=0,
                # KLUCZOWA ZMIANA: Używamy lambda, aby przekazać UNIKALNY obiekt 'btn' do funkcji
                command=lambda current_button=btn: self.toggle_button(current_button)
            )
            btn.pack(pady=5)
            
            # Inicjujemy stan dla tego konkretnego obiektu przycisku
            self.button_states[btn] = True 

    def toggle_button(self, button_object):
        # Sprawdzamy stan klikniętego obiektu w słowniku
