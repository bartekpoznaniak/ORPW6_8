...
class SystemRow(ctk.CTkFrame):
    def __init__(self, parent, name, has_slider=False, has_button=False, has_switch=True, callback=None):
        ...
        if has_button:
            self.btn = MomentaryButton(self.controls_cont, text="PUSH", command=self._on_button_push)
            col = 2 if has_slider else 0
            self.btn.grid(row=0, column=col, padx=5, sticky="e")

        # 3. Przełącznik (Switch) - FIX: Usunięto tk.Label z bg=self.cget("bg")
        self.switch = None
        if has_switch:
            ...
            # Tworzymy pusty kontener o stałej szerokości zamiast Labela
            placeholder = ctk.CTkFrame(self, width=60, height=30, fg_color="transparent")
            placeholder.grid(row=0, column=2, padx=10, pady=10)
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.systemy_lista = [
            ("Zasilanie Główne",  False,   False,   True),
            ("Wyrzutnie Alfa",    True,    True,    True),
            ("Główny Reset",      False,   True,    False),
            ("Moc Reaktora",      True,    False,   False),
            ("System Chłodzenia", True,    False,   True),
            ("Sygnał Bojowy",     False,   True,    True),
            ("Autodestrukcja",    False,   True,    True)
        ]

        ctk.CTkLabel(self, text="MODULARNY PANEL STEROWANIA 2026", font=("Arial", 22, "bold")).pack(pady=20)
